...
def updateBoatPosition(connector, cursor, table_name ,coordinates):
    update_boat_query = "insert into {} (id_region, Geometry) values (1, GeomFromText('POINT({} {})',4326))".format(table_name, coordinates[0], coordinates[1])
    print("yes")
...
def init_db (db, table_name):
    ...
    if os.path.exists(db):
        ...
        existence = test_existence_table(db,table_name)
        if existence == 0:
            print('pas de table')
            pass
        else:
    ...
    cursor.execute("CREATE TABLE {} (id_region INTEGER)".format(table_name))
    cursor.execute("SELECT AddGeometryColumn( '{}', 'Geometry', 4326, 'POINT');".format(table_name))
...
path = "db_boat.sqlite"
#connector, cursor =  connectToDB(path)
